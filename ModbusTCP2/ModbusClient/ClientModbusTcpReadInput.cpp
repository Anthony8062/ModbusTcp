...
// Singleton
//----------------------------------------------------------------------------------------------------
CClientModbusTcpReadInput* CClientModbusTcpReadInput::m_pInstance = nullptr;
CClientModbusTcpReadInput* CClientModbusTcpReadInput::GetInstance()
{
	return m_pInstance ? m_pInstance : (m_pInstance = new CClientModbusTcpReadInput);
}

void CClientModbusTcpReadInput::DelInstance()
{
	if (m_pInstance)
	{
		delete m_pInstance;
...
CClientModbusTcpReadInput::CClientModbusTcpReadInput(void)
{
	InitInfo();
}
...
void CClientModbusTcpReadInput::InitInfo()
{
...
bool CClientModbusTcpReadInput::SocketCleanUp()
{
...
bool CClientModbusTcpReadInput::Run_Socket()
{
	//-----------------------------------------------------------------------
	...
	{
		printf("Device Read Input::ConnectNonBlock() Error\n");
		return SocketCleanUp();
	}
...
bool CClientModbusTcpReadInput::ConnectNonBlocking()
{
	int ret = 0;
	int err = 0;
	int len = 0;
	...
	if (connect(client_sock, (struct sockaddr*) & sock_addr, sizeof(sock_addr)) < 0)
	{
		...
		{
			printf("Device Read Input::connect() err failed : %d\n", err);
			return SocketCleanUp();
		}

		if ((ret = select(client_sock + 1, &fdset, NULL, NULL, &timeout)) <= 0)
		{
			printf("Device Read Input::connect() select() Error\n");
			return SocketCleanUp();
		}
		...
		if (FD_ISSET(client_sock, &fdset))
		{
			...
			if (getsockopt(client_sock, SOL_SOCKET, SO_ERROR, error, &len) < 0) // Solar is pending error
				printf("Device Read Input::connect() getsockopt() Error\n");
			return SocketCleanUp();
		}
		else
		{
			printf("Device Read Input::select error: sockfd not set\n");
			return SocketCleanUp();
		}
	}
...
bool CClientModbusTcpReadInput::ReadOrWrite()
{
	//-----------------------------------------------------------------------          //-----------------------------------------------------------------------
...
unsigned short CClientModbusTcpReadInput::ReverseByteOrder(unsigned short value)
{
...
void CClientModbusTcpReadInput::PrintHexa(char* buff, size_t len)
{
	size_t i;
	for (i = 0; i < len; i++)
	{
...
unsigned short CClientModbusTcpReadInput::CreateCRC16(char* buff, size_t len) // Checksum
{
